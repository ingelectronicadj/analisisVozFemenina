...
%Tomamos una muestra de la senal desde 1801 y la montamos sobre una ventana Hamming de
%200 muestras
figure(1), subplot(2,1,1);plot(x); 
title('Seccion de audio para realizar analisis de la S'); 
...
title('Seccion de audio para realizar analisis de la E'); hold on;
[N,F,A,W] = firpmord([50 100 400 450], [0 1 0], [1 1 1]/200,fs);%#Coeficientes en el filtro, frecuencia, amplitud, ponderaciones de cada banda
% firpmord([frecuencias], ganancias, rizado) -- Parks-McClellan optimal FIR filter order estimation
h = firpm(N,F,A,W);
x = conv (h,x);
...
figure(2),plot(z);title('Representacion grafica de la energia'); 
grid on; grid minor;